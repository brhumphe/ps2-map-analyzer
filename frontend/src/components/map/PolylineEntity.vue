...
const createPolyline = () => {
  ...
  if (props.points.length < 2) {
    console.error(`PolylineEntity[${props.id}]: Insufficient points for polyline (${props.points.length})`)
    return
  }
...
const updatePoints = (newPoints: L.LatLng[]) => {
  ...
  if (newPoints.length < 2) {
    console.error(`PolylineEntity[${props.id}]: Insufficient points for update (${newPoints.length})`)
    return
  }
